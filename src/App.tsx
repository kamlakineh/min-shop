...
import { 
  Heart, ShoppingBag, Search, Phone as PhoneIcon, Mail, MapPin, 
  ChevronRight, Star, Youtube, Share2, Clipboard, ArrowLeft, ArrowRight, Send, 
  Clock, ShieldAlert, CheckCircle2, ShoppingCart, User, Truck, HelpCircle, 
  Grid, List, FileText, ChevronDown, Check, X 
} from "lucide-react";
...
import { 
  Phone, PhoneVariant, Brand, Category, Order, Coupon, ContactMessage, Notification, 
  WebsiteSettings, CartItem 
} from "./types";
...
export default function App() {
  ...
  const [selectedStorageFilter, setSelectedStorageFilter] = useState<string>("");
  const [selectedPriceMaxFilter, setSelectedPriceMaxFilter] = useState<number>(200000);
  const [selected5GFilter, setSelected5GFilter] = useState<boolean>(false);
  const [selectedInStockFilter, setSelectedInStockFilter] = useState<boolean>(false);
  ...
  const handleSelectColor = (color: string) => {
    ...
    const exactMatch = selectedProduct.variants.find(v => v.color === color && v.storage === selectedVariantStorage && v.ram === selectedVariantRam);
    if (!exactMatch) {
      const partialMatch = selectedProduct.variants.find(v => v.color === color && v.storage === selectedVariantStorage)
        || selectedProduct.variants.find(v => v.color === color && v.ram === selectedVariantRam)
  ...
  const handleSelectStorage = (store: string) => {
    ...
    const exactMatch = selectedProduct.variants.find(v => v.color === selectedVariantColor && v.storage === store && v.ram === selectedVariantRam);
    if (!exactMatch) {
      const partialMatch = selectedProduct.variants.find(v => v.storage === store && v.color === selectedVariantColor)
        || selectedProduct.variants.find(v => v.storage === store && v.ram === selectedVariantRam)
  ...
  const handleSelectRam = (ram: string) => {
    ...
    const exactMatch = selectedProduct.variants.find(v => v.color === selectedVariantColor && v.storage === selectedVariantStorage && v.ram === ram);
    if (!exactMatch) {
      const partialMatch = selectedProduct.variants.find(v => v.ram === ram && v.color === selectedVariantColor)
        || selectedProduct.variants.find(v => v.ram === ram && v.storage === selectedVariantStorage)
  ...
  const handleAdminLogout = async () => {
    if (adminToken) {
      try {
        await axios.post("/api/auth/logout", {}, { headers: { Authorization: `Bearer ${adminToken}` } });
      } catch (e) {}
    }
    setAdminToken(null);
    localStorage.removeItem("admin_token");
  ...
  // ----------------------------------------------------
  const handleAddToCart = (phone: Phone, e?: React.MouseEvent, variant?: PhoneVariant) => {
    if (e) e.stopPropagation();
    
    const variantId = variant?.id;
    // Find if already exists
    const existingIdx = cart.findIndex(item => item.phoneId === phone.id && item.variantId === variantId);
    if (existingIdx > -1) {
      const newCart = [...cart];
  ...
  const handleRemoveFromCart = (phoneId: string, variantId?: string) => {
    const newCart = cart.filter(item => !(item.phoneId === phoneId && item.variantId === variantId));
    setCart(newCart);
  };

  const handleCartQuantityChange = (phoneId: string, delta: number, variantId?: string) => {
  ...
  const handleCheckoutSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(err.response?.data?.error || "Failed to submit checkout. Please try again.");
    }
  };
  ...
  const handleTrackOrder = async (e: React.FormEvent) => {
    ...
    try {
      const res = await axios.get(`/api/orders/track?phone=${trackPhone.trim()}&orderNumber=${trackOrderNumber.trim()}`);
      setTrackResult(res.data);
    } catch (err: any) {
      setTrackError(err.response?.data?.error || "Order not found. Please double check details.");
    }
  };
  ...
  const handleContactSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setContactSuccessMsg("Your message was sent successfully! Our sales staff will call you shortly.");
  ...
  const getFilteredPhones = () => {
    return phones.filter(p => {
      ...
      if (searchQuery) {
        ...
        const matchesBrand = p.brand?.name.toLowerCase().includes(query) || false;
      ...
      // 3. Brand select
      if (selectedBrandFilter && p.brandId !== selectedBrandFilter) return false;
      ...
      if (selectedRamFilter) {
        const hasRam = p.variants.some(v => v.ram.toLowerCase() === selectedRamFilter.toLowerCase());
        if (!hasRam) return false;
      }

      // 6. Storage select
      if (selectedStorageFilter) {
        const hasStorage = p.variants.some(v => v.storage.toLowerCase() === selectedStorageFilter.toLowerCase());
  ...
  return (
    <div className="min-h-screen bg-white text-black flex flex-col selection:bg-blue-600 selection:text-white">
          ...
          <div className="space-y-16 pb-16 animate-fade-in select-none">
              ...
              <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 lg:py-28 grid grid-cols-1 lg:grid-cols-2 items-center gap-12">
                <div className="space-y-6">
                  ...
                  <p className="text-sm text-gray-400 font-sans max-w-lg leading-relaxed">
                    Browse authentic Apple, Samsung, Google Pixel, Xiaomi, and Tecno devices. Fully customized delivery across all sub-cities in Addis Ababa within 24 hours. Duty-paid and locally verified.
                  </p>
                ...
                <div className="hidden lg:flex justify-center relative">
                  <div className="relative shadow-lg p-6 bg-white w-96 flex flex-col space-y-4 border-none">
                    ...
                    <div className="flex justify-between items-end border-t border-gray-100 pt-4">
                      <div>
                        <p className="text-[10px] font-mono text-gray-500 font-bold uppercase">APPLE PLATINUM</p>
                        <h4 className="font-display font-extrabold text-sm text-black">iPhone 15 Pro Max</h4>
                      </div>
                      <button
                        onClick={() => {
                          const ip = phones.find(p => p.name.includes("iPhone 15 Pro"));
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="flex flex-col sm:flex-row justify-between items-baseline border-b border-gray-100 pb-4 mb-8">
                <div>
                  ...
                  <p className="text-xs text-gray-500 font-mono mt-0.5 uppercase">LIMITED 24-HOUR PROMO STOCK</p>
                </div>
                <button
                  onClick={() => {
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {phones.filter(p => p.isFlashSale).slice(0, 4).map((phone) => (
                  <ProductCard
                    key={phone.id}
                    phone={phone}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="shadow-sm hover:shadow-md bg-white p-6 flex flex-col justify-between min-h-[220px] transition-all border-none">
                  <div className="space-y-2">
                    <span className="font-mono text-[9px] font-bold text-blue-600 tracking-wider">PRESTIGE CHOICE</span>
                    <h3 className="font-display font-extrabold text-lg text-black">APPLE iPHONES</h3>
                    <p className="text-xs text-gray-500">Discover direct factory-certified titanium devices with full apple insurance.</p>
                  </div>
                  <button
                    onClick={() => {
                      const ipCat = categories.find(c => c.name.toLowerCase().includes("apple"));
                ...
                <div className="shadow-sm hover:shadow-md bg-white p-6 flex flex-col justify-between min-h-[220px] transition-all border-none">
                  <div className="space-y-2">
                    <span className="font-mono text-[9px] font-bold text-blue-600 tracking-wider">FLAGSHIP DOMINATOR</span>
                    <h3 className="font-display font-extrabold text-lg text-black">SAMSUNG GALAXY</h3>
                    <p className="text-xs text-gray-500">Unleash galaxy AI features and premium display panels with local brand backup.</p>
                  </div>
                  <button
                    onClick={() => {
                      const samCat = categories.find(c => c.name.toLowerCase().includes("samsung"));
                ...
                <div className="shadow-sm hover:shadow-md bg-white p-6 flex flex-col justify-between min-h-[220px] transition-all border-none">
                  <div className="space-y-2">
                    <span className="font-mono text-[9px] font-bold text-blue-600 tracking-wider">DAILY UTILITIES</span>
                    <h3 className="font-display font-extrabold text-lg text-black">MOBILE ACCESSORIES</h3>
                    <p className="text-xs text-gray-500">Chargers, smart watches, case shields and power delivery hubs from Anker and Apple.</p>
                  </div>
                  <button
                    onClick={() => {
                      const accCat = categories.find(c => c.name.toLowerCase().includes("access"));
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="flex flex-col sm:flex-row justify-between items-baseline border-b border-gray-100 pb-4 mb-8">
                <div>
                  ...
                  <p className="text-xs text-gray-500 font-mono mt-0.5 uppercase">VERIFIED HIGH-PERFORMANCE SMARTPHONES</p>
                </div>
                <button
                  onClick={() => {
                    setSelectedCategoryFilter("");
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {phones.filter(p => p.isFeatured && !p.isFlashSale).slice(0, 4).map((phone) => (
                  <ProductCard
                    key={phone.id}
                    phone={phone}
                    onViewDetails={onViewProductDetails}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-10">
                  <div className="shadow-sm hover:shadow bg-white p-6 flex flex-col justify-between text-left transition-all border-none">
                    <p className="text-xs text-gray-600 font-sans italic">
                      "Absolutely genuine iPhone 15 Pro Max. Delivered to my office at Dembel within 2 hours. Checked the IMEI on apple site before payment. Highly professional service!"
                    </p>
                    <div className="mt-4 pt-4 border-t border-gray-100 flex justify-between items-center text-xs font-mono">
                      <span className="font-bold text-black">Abebe K. (Bole)</span>
                  ...
                  <div className="shadow-sm hover:shadow bg-white p-6 flex flex-col justify-between text-left transition-all border-none">
                    <p className="text-xs text-gray-600 font-sans italic">
                      "I bought a Tecno Spark 20 Pro from their catalog. Best budget phone. The delivery dispatcher was very kind and patient while I booted and tested the camera."
                    </p>
                    <div className="mt-4 pt-4 border-t border-gray-100 flex justify-between items-center text-xs font-mono">
                      <span className="font-bold text-black">Selamawit T. (Yeka)</span>
                      <span className="text-yellow-500 font-bold">★★★★★</span>
                    </div>
                  </div>
                  ...
                  <div className="shadow-sm hover:shadow bg-white p-6 flex flex-col justify-between text-left transition-all border-none">
                    <p className="text-xs text-gray-600 font-sans italic">
                      "Awesome! Used WELCOME10 code and saved 3000 Birr on my Galaxy S24 Ultra. The local 1 year warranty terms are solid. Recommend EthioPhone to anyone."
                    </p>
                    <div className="mt-4 pt-4 border-t border-gray-100 flex justify-between items-center text-xs font-mono">
                      <span className="font-bold text-black">Yonas M. (Kirkos)</span>
                      <span className="text-yellow-500 font-bold">★★★★★</span>
                    </div>
                  </div>
                </div>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="border-b border-gray-100 pb-4 mb-8">
                ...
                <p className="text-xs text-gray-500 font-mono mt-0.5 uppercase">PHONE COMPILATIONS & LOCAL BUYER GUIDES</p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {blogs.slice(0, 2).map((blog) => (
                  <div 
                    key={blog.id} 
                    onClick={() => { setSelectedBlog(blog); setActivePage("blog-detail"); window.scrollTo(0,0); }}
                  ...
                  >
                    ...
                    <div className="p-4 flex flex-col flex-grow space-y-2 text-left">
                      ...
                      <span className="text-[10px] font-mono text-gray-400 mt-auto pt-2">
                        📅 {new Date(blog.createdAt).toLocaleDateString()} | BY {blog.author.toUpperCase()}
                      </span>
                    </div>
                  </div>
                ))}
        ...
        {activePage === "shop" && (
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in select-none">
            <div className="border-b border-gray-100 pb-4 mb-6">
              ...
              <p className="text-xs text-gray-500 font-mono mt-0.5">INSTANT FILTERS & VERIFIED PRICING IN ETB</p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
              ...
              <div className="lg:col-span-1 shadow-sm bg-white p-4 space-y-6 border-none">
                ...
                <div className="space-y-1 text-xs">
                  <label className="font-mono font-bold text-gray-500 block">CATEGORY</label>
                  <select
                ...
                {/* Brand Filter */}
                <div className="space-y-1 text-xs">
                  <label className="font-mono font-bold text-gray-500 block">BRAND</label>
                  <select
                ...
                {/* RAM filter */}
                <div className="space-y-1 text-xs">
                  <label className="font-mono font-bold text-gray-500 block">RAM SIZE</label>
                  <select
                ...
                {/* Storage filter */}
                <div className="space-y-1 text-xs">
                  <label className="font-mono font-bold text-gray-500 block">STORAGE SPACE</label>
                  <select
                ...
                <div className="space-y-2 text-xs">
                  <div className="flex justify-between font-mono font-bold text-gray-500">
                    <span>MAX PRICE</span>
                    <span className="text-black font-mono">{selectedPriceMaxFilter.toLocaleString()} ETB</span>
                  </div>
                  <input
                    type="range"
                    min={5000}
                    ...
                    onChange={(e) => setSelectedPriceMaxFilter(Number(e.target.value))}
                ...
                <div className="space-y-3 font-mono text-xs font-bold pt-2 border-t border-dashed border-gray-200">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer text-red-600">
                    <input
                      ...
                      onChange={(e) => setSelectedInStockFilter(e.target.checked)}
                      className="accent-blue-600 border-black"
                    />
              ...
              <div className="lg:col-span-3">
                {getFilteredPhones().length > 0 ? (
                  ...
                  <div className="shadow-sm bg-white p-12 text-center flex flex-col items-center justify-center space-y-4 border-none">
                    ...
                    <h3 className="font-display font-bold text-base text-black">NO MATCHING PHONES IN STOCK</h3>
                    <p className="text-xs text-gray-500 max-w-md leading-relaxed">
                      We couldn't find any products matching those parameters. Try resetting filters or searching with less specific keywords.
                    </p>
                  </div>
                )}
              </div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in text-left">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 select-none">
              ...
              <div className="space-y-4">
                <div className="shadow-sm w-full h-96 bg-gray-50 flex items-center justify-center p-4 border-none">
                  <img
                    src={selectedProduct.images?.[0]?.url || "https://images.unsplash.com/photo-1511707171634-5f897ff02aa9?w=600&h=600&fit=crop&q=80"}
                ...
                <div className="flex gap-4">
                  {selectedProduct.images?.map((img, i) => (
                    ...
                    >
                      <img src={img.url} className="h-full object-contain" referrerPolicy="no-referrer" />
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-xs text-gray-500 font-mono mt-1">MODEL REF: {selectedProduct.model}</p>
                </div>
                ...
                <div className="bg-gray-50 p-4 flex justify-between items-center shadow-sm border-none">
                  <div>
                    <span className="text-[10px] font-mono text-gray-500 block">FINAL PRICE INBIRR</span>
                    <span className="font-mono text-xl font-bold text-black">
                      {((selectedProduct.price * (1 - selectedProduct.discount / 100)) + (activeSelectedVariant ? activeSelectedVariant.priceModifier : 0)).toLocaleString()} ETB
                    </span>
                  </div>
                  {selectedProduct.discount > 0 && (
                    <div className="text-right">
                      <span className="text-[9px] font-mono text-red-500 block font-bold">SAVE {selectedProduct.discount}%</span>
                      <span className="font-mono text-sm text-gray-400 line-through">
                        {(selectedProduct.price + (activeSelectedVariant ? activeSelectedVariant.priceModifier : 0)).toLocaleString()} ETB
                      </span>
                    </div>
                  )}
                </div>
                ...
                <div className="text-xs font-mono">
                  {activeSelectedVariant ? (
                    activeSelectedVariant.stock > 0 ? (
                      <span className="text-green-600 font-semibold">✔️ {selectedVariantColor} - {selectedVariantStorage} ({selectedVariantRam} RAM) In Stock - Ready for bole dispatcher courier delivery ({activeSelectedVariant.stock} left)</span>
                    ) : (
                      <span className="text-red-600 font-bold uppercase">❌ {selectedVariantColor} - {selectedVariantStorage} ({selectedVariantRam} RAM) Temp Out of Stock - Dispatch Delayed</span>
                    )
                  ) : selectedProduct.stock > 0 ? (
                    <span className="text-green-600 font-semibold">✔️ In Stock - Ready for bole dispatcher courier delivery ({selectedProduct.stock} left)</span>
                  ) : (
                    <span className="text-red-600 font-bold uppercase">❌ Temp Out of Stock - Dispatch Delayed</span>
                  )}
                </div>
                ...
                {selectedProduct.variants && selectedProduct.variants.length > 0 && (
                  <div className="space-y-4 border-t border-b border-gray-100 py-4 text-xs font-mono select-none">
                    ...
                    <div className="space-y-1.5">
                      <span className="font-bold text-gray-500 block">CHOOSE DEVICE COLOR</span>
                      <div className="flex flex-wrap gap-2">
                        {Array.from<string>(new Set(selectedProduct.variants.map(v => v.color))).map(color => (
                          <button
                            key={color}
                            onClick={() => handleSelectColor(color)}
                            className={`px-3 py-1 font-semibold text-xs transition-all border-none shadow-sm ${
                              selectedVariantColor === color ? "bg-blue-600 text-white shadow-md font-bold" : "bg-gray-100 text-black hover:bg-gray-200"
                            }`}
                          >
                            {color}
                          </button>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        <span className="font-bold text-gray-500 block">STORAGE VARIANT</span>
                        <div className="flex flex-wrap gap-2">
                          {Array.from<string>(new Set(selectedProduct.variants.map(v => v.storage))).map(store => (
                            <button
                              key={store}
                              onClick={() => handleSelectStorage(store)}
                              className={`px-3 py-1 font-semibold text-xs transition-all border-none shadow-sm ${
                                selectedVariantStorage === store ? "bg-blue-600 text-white shadow-md font-bold" : "bg-gray-100 text-black hover:bg-gray-200"
                              }`}
                            >
                              {store}
                            </button>
                      ...
                      <div className="space-y-1.5">
                        <span className="font-bold text-gray-500 block">SYSTEM RAM</span>
                        <div className="flex flex-wrap gap-2">
                          {Array.from<string>(new Set(selectedProduct.variants.map(v => v.ram))).map(ram => (
                            <button
                              key={ram}
                              onClick={() => handleSelectRam(ram)}
                              className={`px-3 py-1 font-semibold text-xs transition-all border-none shadow-sm ${
                                selectedVariantRam === ram ? "bg-blue-600 text-white shadow-md font-bold" : "bg-gray-100 text-black hover:bg-gray-200"
                              }`}
                            >
                              {ram}
                            </button>
                ...
                <div className="flex flex-col sm:flex-row gap-4 pt-2 font-mono text-xs font-bold">
                  <button
                    onClick={() => {
                      const finalStock = activeSelectedVariant ? activeSelectedVariant.stock : selectedProduct.stock;
                      if (finalStock > 0) {
                        handleAddToCart(selectedProduct, undefined, activeSelectedVariant || undefined);
                        setActivePage("cart");
                      }
                    }}
                    disabled={(activeSelectedVariant ? activeSelectedVariant.stock <= 0 : selectedProduct.stock <= 0)}
                    className={`flex-1 py-3 hover:bg-blue-600 hover:text-white transition-all text-center border-none shadow-sm ${
                      (activeSelectedVariant ? activeSelectedVariant.stock <= 0 : selectedProduct.stock <= 0) ? "opacity-50 cursor-not-allowed bg-gray-100 text-gray-400" : "bg-gray-100 text-black"
                    }`}
                  ...
                  <button
                    ...
                    className={`flex-1 py-3 text-white transition-all border-none shadow-sm hover:shadow ${
                      (activeSelectedVariant ? activeSelectedVariant.stock <= 0 : selectedProduct.stock <= 0)
                ...
                <div className="border border-dashed border-gray-300 p-3.5 text-xs text-gray-600 leading-relaxed bg-gray-50">
                  <span className="font-mono font-bold text-black uppercase block mb-1">🛡️ WARRANTY & EXCHANGE PROMISE</span>
                  {selectedProduct.warranty}. Fast local troubleshooting via our Bole Road showroom. Full hardware exchange in case of out-of-box fault.
                </div>
              </div>
            ...
            <div className="mt-16 bg-white shadow-sm border-none">
              ...
              <div className="p-6 space-y-6">
                <div>
                  <h4 className="font-display font-bold text-sm text-black border-b border-gray-100 pb-2 mb-3 uppercase">PRODUCT REVIEW OUTLINE</h4>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-xs font-sans">
                  ...
                  <div className="bg-gray-50 p-4 border-none shadow-sm">
                    <h5 className="font-mono font-bold text-gray-500 border-b border-gray-200 pb-1.5 mb-2 uppercase text-[10px]">DISPLAY PERFORMANCE</h5>
                    <div className="space-y-1.5 font-mono text-[11px] text-gray-700">
                      <p><span className="font-bold text-black">DISPLAY SCREEN:</span> 6.7" OLED / AMOLED</p>
                      <p><span className="font-bold text-black">REFRESH RATE:</span> 120Hz ProMotion</p>
                      <p><span className="font-bold text-black">DUAL SIM SLOT:</span> Yes Support</p>
                      <p><span className="font-bold text-black">BOX CONTENTS:</span> Device Charger, USB-C Cable</p>
                    </div>
                  </div>

                  {/* Hardware details */}
                  <div className="bg-gray-50 p-4 border-none shadow-sm">
                    <h5 className="font-mono font-bold text-gray-500 border-b border-gray-200 pb-1.5 mb-2 uppercase text-[10px]">CAMERA & HARDWARE</h5>
                    <div className="space-y-1.5 font-mono text-[11px] text-gray-700">
                      <p><span className="font-bold text-black">REAR CAMERA:</span> 108MP Quad HD Matrix</p>
                      <p><span className="font-bold text-black">CHARGING PORT:</span> USB Type C standard</p>
                      <p><span className="font-bold text-black">5G CARRIER SUPPORT:</span> Ready and Verified</p>
                      <p><span className="font-bold text-black">LOCAL WARRANTY:</span> 1 Year Carlcare Local</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="mt-12 space-y-6">
              ...
              <form onSubmit={handleReviewSubmit} className="shadow-sm bg-gray-50 p-4 space-y-4 max-w-xl text-xs border-none">
                <h4 className="font-mono font-bold text-black uppercase">SUBMIT YOUR VERIFIED REVIEW</h4>
                {reviewSuccessMsg && (
                  <p className="bg-green-50 border border-green-600 text-green-600 p-2 font-mono">{reviewSuccessMsg}</p>
                )}
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="font-mono font-bold text-gray-500 block">FULL NAME *</label>
                    <input
                      type="text"
                      required
                      value={reviewName}
                  ...
                  <div className="space-y-1">
                    <label className="font-mono font-bold text-gray-500 block">DEVICE RATING *</label>
                    <select
                      value={reviewRating}
                ...
                <div className="space-y-1">
                  <label className="font-mono font-bold text-gray-500 block">YOUR DETAILED REVIEW *</label>
                  <textarea
                    required
                    rows={2.5}
              ...
              <div className="space-y-3 font-sans text-xs">
                {selectedProduct.reviews?.map((review) => (
                  <div key={review.id} className="shadow-sm bg-white p-4 space-y-2 border-none">
                    ...
                    <p className="text-[10px] font-mono text-gray-400">POSTED: {new Date(review.createdAt || Date.now()).toLocaleDateString()}</p>
                  </div>
                ))}
                {(!selectedProduct.reviews || selectedProduct.reviews.length === 0) && (
                  <p className="text-gray-400 italic">No reviews yet for this model. Be the first to review!</p>
                )}
              </div>
            </div>
        ...
        {activePage === "cart" && (
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in text-left select-none">
            <div className="border-b border-gray-100 pb-4 mb-8">
              ...
              <p className="text-xs text-gray-500 font-mono mt-0.5">LOCAL COOKIE/BROWSER PERSISTED BASKET</p>
            </div>
            ...
            {cart.length > 0 ? (
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                ...
                <div className="lg:col-span-2 space-y-4">
                  {cart.map((item) => {
                    let itemUnitPrice = item.phone.price * (1 - item.phone.discount / 100);
                    if (item.selectedVariant) {
                      itemUnitPrice += item.selectedVariant.priceModifier;
                    }
                    const itemTotalPrice = itemUnitPrice * item.quantity;
                    return (
                      <div key={`${item.phoneId}-${item.variantId || 'base'}`} className="shadow-sm bg-white p-4 flex gap-4 items-center relative border-none">
                        <button
                          onClick={() => handleRemoveFromCart(item.phoneId, item.variantId)}
                        ...
                        <div className="flex-1 font-sans text-xs space-y-1">
                          <h4 className="font-bold text-black text-sm">{item.phone.name}</h4>
                          ...
                          <p className="text-[10px] font-mono text-gray-400 uppercase">MODEL: {item.phone.model}</p>
                          <p className="font-mono font-bold text-black">
                            {itemUnitPrice.toLocaleString()} ETB {item.quantity > 1 && <span className="text-[10px] text-gray-400 font-normal">({itemTotalPrice.toLocaleString()} ETB total)</span>}
                          </p>
                          
                          {/* Qty increment controls */}
                          <div className="flex items-center gap-2 pt-2 text-xs font-mono font-bold">
                            <button
                              onClick={() => handleCartQuantityChange(item.phoneId, -1, item.variantId)}
                ...
                <div className="lg:col-span-1 shadow-sm p-6 bg-gray-50 flex flex-col justify-between h-fit space-y-6 border-none">
                  <div className="space-y-4">
                    ...
                    <form onSubmit={handleApplyCoupon} className="space-y-2 text-xs">
                      <label className="font-mono font-bold text-gray-500 block">APPLY PROMO CODE</label>
                      ...
                      {couponError && <p className="text-red-600 font-mono text-[10px]">{couponError}</p>}
                      {appliedCoupon && (
                        <div className="flex justify-between items-center bg-green-50 text-green-700 p-1.5 font-mono text-[10px] font-semibold shadow-sm border-none">
                          ...
                          <button onClick={() => setAppliedCoupon(null)} className="text-red-600 hover:underline text-[9px]">REMOVE</button>
                        </div>
                      )}
                    </form>
                    ...
                    <div className="space-y-2 text-xs font-mono pt-4 border-t border-dashed border-gray-300">
                      <div className="flex justify-between text-gray-500">
                        ...
                        <span className="text-black">{getCartSubtotal().toLocaleString()} ETB</span>
                      </div>
                      {getDiscountVal() > 0 && (
                      ...
                      <div className="flex justify-between text-gray-500 border-b border-dashed border-gray-300 pb-2">
                        ...
                        <span className="text-black">{getDeliveryCost().toLocaleString()} ETB</span>
                      </div>
                      <div className="flex justify-between text-sm font-bold text-black pt-1">
                        <span>GRAND TOTAL:</span>
                        <span className="text-blue-600">{getGrandTotal().toLocaleString()} ETB</span>
                      </div>
                    </div>
                  </div>
              ...
              <div className="shadow-sm bg-white p-12 text-center flex flex-col items-center justify-center space-y-4 border-none">
                ...
                <h3 className="font-display font-bold text-base text-black">YOUR CART IS EMPTY</h3>
                <p className="text-xs text-gray-500 max-w-sm leading-relaxed">
                  You haven't added any phones or accessories to your cart yet. Explore our latest smartphones to get started.
                </p>
                <button
                  onClick={() => setActivePage("shop")}
        ...
        {activePage === "favorites" && (
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in text-left select-none">
            <div className="border-b border-gray-100 pb-4 mb-8">
              ...
              <p className="text-xs text-gray-500 font-mono mt-0.5">LOCAL BROWSER SHORTLIST</p>
            </div>
            ...
            {favorites.length > 0 ? (
              ...
              <div className="shadow-sm bg-white p-12 text-center flex flex-col items-center justify-center space-y-4 border-none">
                ...
                <h3 className="font-display font-bold text-base text-black">NO FAVORITE PHONES SAVED</h3>
                <p className="text-xs text-gray-500 max-w-sm leading-relaxed">
                  You can bookmark smartphones and accessories in your browser shortlist by clicking the heart icon on any device card.
                </p>
                <button
                  onClick={() => setActivePage("shop")}
                  className="bg-black text-white text-xs font-mono font-bold px-4 py-2 hover:bg-blue-600 border-none shadow-sm hover:shadow"
        ...
        {activePage === "checkout" && (
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in text-left select-none">
            <div className="border-b border-gray-100 pb-4 mb-8">
              ...
              <p className="text-xs text-gray-500 font-mono mt-0.5">METRO ADDIS COURIER DISPATCH REGISTRY</p>
            </div>

            <form onSubmit={handleCheckoutSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-12 font-sans">
              ...
              <div className="lg:col-span-2 shadow-sm p-6 bg-white space-y-6 border-none">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                  <div className="space-y-1">
                    <label className="font-mono font-bold text-gray-500 block">FULL RECIPIENT NAME *</label>
                    <input
                      type="text"
                      required
                  ...
                  <div className="space-y-1">
                    <label className="font-mono font-bold text-gray-500 block">PHONE NUMBER *</label>
                    <input
                      type="tel"
                      required
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                  <div className="space-y-1">
                    <label className="font-mono font-bold text-gray-500 block">ALTERNATIVE PHONE NUMBER</label>
                    <input
                      type="tel"
                      placeholder="e.g. 0922334455"
                      value={checkoutForm.altPhoneNumber}
                      onChange={(e) => setCheckoutForm({ ...checkoutForm, altPhoneNumber: e.target.value })}
                      className="w-full bg-gray-50 px-2 py-1.5 font-mono border-none shadow-sm"
                    />
                  </div>
                  <div className="space-y-1">
                    <label className="font-mono font-bold text-gray-500 block">EMAIL ADDRESS (OPTIONAL)</label>
                    <input
                      type="email"
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs font-mono">
                  <div className="space-y-1">
                    <label className="font-bold text-gray-500 block">REGION *</label>
                    <input
                      type="text"
                      required
                  ...
                  <div className="space-y-1">
                    <label className="font-bold text-gray-500 block">CITY *</label>
                    <input
                      type="text"
                      required
                  ...
                  <div className="space-y-1">
                    <label className="font-bold text-gray-500 block">SUBCITY *</label>
                    <input
                      type="text"
                      required
                      placeholder="e.g. Bole"
                      value={checkoutForm.subCity}
                      onChange={(e) => setCheckoutForm({ ...checkoutForm, subCity: e.target.value })}
                      className="w-full bg-gray-50 px-2 py-1.5 border-none shadow-sm"
                    />
                  </div>
                  <div className="space-y-1">
                    <label className="font-bold text-gray-500 block">WOREDA *</label>
                    <input
                      type="text"
                      required
                      placeholder="e.g. 03"
                      value={checkoutForm.woreda}
                      onChange={(e) => setCheckoutForm({ ...checkoutForm, woreda: e.target.value })}
                      className="w-full bg-gray-50 px-2 py-1.5 border-none shadow-sm"
                    />
                  </div>
                </div>
                ...
                <div className="text-xs space-y-1">
                  <label className="font-mono font-bold text-gray-500 block">DETAILED STREET DIRECTION & BUILDING NAME *</label>
                  <input
                    type="text"
                    required
                ...
                <div className="text-xs space-y-1">
                  <label className="font-mono font-bold text-gray-500 block">SPECIAL DELIVERY COURIER NOTES</label>
                  <textarea
                    rows={2}
                ...
                <div className="space-y-3 font-mono text-xs font-bold pt-4 border-t border-gray-100">
                  <span className="text-gray-500 block uppercase">CHOOSE SETTLEMENT METHOD</span>
                  
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <label className={`p-3 flex items-center gap-3 cursor-pointer shadow-sm border-none transition-all ${
                      checkoutForm.paymentMethod === "cod" ? "bg-blue-50/70 shadow-md ring-1 ring-blue-600/20" : "bg-white"
                    }`}>
                      <input
                        type="radio"
                        name="payment_method"
                      ...
                      />
                      <div>
                        <p className="text-black uppercase text-xs font-bold">CASH ON DELIVERY</p>
                        <p className="text-[9px] text-gray-500 font-sans leading-tight">Pay cash or mobile money directly to dispatcher.</p>
                      </div>
                    </label>

                    <label className={`p-3 flex items-center gap-3 cursor-pointer shadow-sm border-none transition-all ${
                      checkoutForm.paymentMethod === "chapa" ? "bg-blue-50/70 shadow-md ring-1 ring-blue-600/20" : "bg-white"
                    }`}>
                      <input
                        type="radio"
                        name="payment_method"
                        checked={checkoutForm.paymentMethod === "chapa"}
                        onChange={() => setCheckoutForm({ ...checkoutForm, paymentMethod: "chapa" })}
                        className="accent-blue-600"
                      />
                      <div>
                        <p className="text-black uppercase text-xs font-bold">CHAPA SECURE</p>
                        <p className="text-[9px] text-gray-500 font-sans leading-tight">Pay instantly using Telebirr, CBE, or Cards.</p>
                      </div>
                    </label>

                    <label className={`p-3 flex items-center gap-3 cursor-pointer shadow-sm border-none transition-all ${
                      checkoutForm.paymentMethod === "bank_transfer" ? "bg-blue-50/70 shadow-md ring-1 ring-blue-600/20" : "bg-white"
                    }`}>
                      <input
                        type="radio"
                        name="payment_method"
                        checked={checkoutForm.paymentMethod === "bank_transfer"}
                        onChange={() => setCheckoutForm({ ...checkoutForm, paymentMethod: "bank_transfer" })}
                        className="accent-blue-600"
                      />
                      <div>
                        <p className="text-black uppercase text-xs font-bold">BANK TRANSFER</p>
                        <p className="text-[9px] text-gray-500 font-sans leading-tight">Prepay directly to our corporate CBE account.</p>
                      </div>
                    </label>
                  </div>
                </div>
              ...
              <div className="lg:col-span-1 shadow-sm p-6 bg-gray-50 flex flex-col justify-between h-fit space-y-6 border-none select-none">
                <div className="space-y-4">
                  ...
                  <div className="space-y-2 text-xs font-mono">
                    ...
                    <div className="flex justify-between text-sm font-bold text-black pt-1">
                      <span>FINAL DUE:</span>
                      <span className="text-blue-600 text-base">{getGrandTotal().toLocaleString()} ETB</span>
                    </div>
                  </div>
                </div>
            ...
            <div className="shadow-md p-8 bg-white space-y-6 border-none">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-gray-500 font-mono">
                  YOUR UNIQUE DISPATCH IDENTIFIER: <span className="font-bold text-blue-600">{successOrder.orderNumber}</span>
                </p>
              </div>

              <div className="shadow-sm p-4 bg-gray-50 text-xs text-left space-y-2 font-mono border-none">
                <p>👤 <span className="font-bold">RECIPIENT:</span> {successOrder.fullName}</p>
                <p>📞 <span className="font-bold">CONTACT NUMBER:</span> {successOrder.phoneNumber}</p>
                <p>📍 <span className="font-bold">DELIVERY HUB:</span> {successOrder.city} - {successOrder.deliveryAddress}</p>
                <p>💳 <span className="font-bold">SETTLEMENT:</span> {successOrder.paymentMethod.toUpperCase()}</p>
              ...
              <p className="text-xs text-gray-600 font-sans leading-relaxed">
                Thank you for shopping with us! Our dispatch courier has allocated your smartphone and will call you to coordinate the delivery routing.
              </p>
        ...
        {activePage === "track" && (
          <div className="max-w-xl mx-auto px-4 py-8 animate-fade-in text-left select-none">
            <div className="border-b border-gray-100 pb-4 mb-8 text-center">
              ...
              <p className="text-xs text-gray-500 font-mono mt-0.5">METRO ADDIS COURIER LOGISTICS TRACKER</p>
            </div>

            <form onSubmit={handleTrackOrder} className="shadow-md p-6 bg-white space-y-4 border-none">
              {trackError && <p className="bg-red-50 border border-red-600 text-red-600 p-2 text-xs font-mono">{trackError}</p>}
              
              <div className="space-y-1 text-xs">
                <label className="font-mono font-bold text-gray-500 block">RECIPIENT PHONE NUMBER *</label>
                <input
                  type="tel"
                  required
                  placeholder="e.g. 0911223344"
              ...
              <div className="space-y-1 text-xs">
                <label className="font-mono font-bold text-gray-500 block">ORDER ID REFERENCE *</label>
                <input
                  type="text"
                  required
            ...
            {trackResult && (
              <div className="mt-8 shadow-sm p-6 bg-gray-50 space-y-6 border-none">
                <div className="flex justify-between items-baseline border-b border-gray-100 pb-3">
                  <h3 className="font-mono font-bold text-xs text-black">TRACKING RESULTS</h3>
                ...
                <div className="space-y-4 font-mono text-[11px] font-bold">
                  <div className="flex justify-between border-b border-gray-200 pb-2 text-xs">
                    ...
                    <span className="text-blue-600 uppercase">{trackResult.status}</span>
                  </div>

                  {/* Visual tracking steps */}
                  <div className="relative pl-6 space-y-6 border-l border-gray-300 py-1">
                    <div className="relative">
                      <div className={`absolute -left-[30px] top-0 w-4 h-4 border border-gray-200 flex items-center justify-center text-[9px] ${
                        ["Pending", "Confirmed", "Preparing", "Ready", "Shipped", "Delivered"].includes(trackResult.status) ? "bg-black text-white" : "bg-white"
                      }`}>✓</div>
                      <p className="text-black uppercase">Pending Confirmation</p>
                      <p className="font-sans font-normal text-gray-500 text-[10px] leading-tight">Order received and waiting for warehouse inventory allocation.</p>
                    </div>

                    <div className="relative">
                      <div className={`absolute -left-[30px] top-0 w-4 h-4 border border-gray-200 flex items-center justify-center text-[9px] ${
                        ["Confirmed", "Preparing", "Ready", "Shipped", "Delivered"].includes(trackResult.status) ? "bg-black text-white" : "bg-white"
                      }`}>✓</div>
                      <p className="text-black uppercase">Order Confirmed</p>
                      <p className="font-sans font-normal text-gray-500 text-[10px] leading-tight">Our staff called and validated the delivery coordinate.</p>
                    </div>

                    <div className="relative">
                      <div className={`absolute -left-[30px] top-0 w-4 h-4 border border-gray-200 flex items-center justify-center text-[9px] ${
                        ["Shipped", "Delivered"].includes(trackResult.status) ? "bg-black text-white" : "bg-white"
                      }`}>✓</div>
                      <p className="text-black uppercase">Dispatched & Transit</p>
                      <p className="font-sans font-normal text-gray-500 text-[10px] leading-tight">The active dispatcher has left the Bole branch towards your destination.</p>
                    </div>

                    <div className="relative">
                      <div className={`absolute -left-[30px] top-0 w-4 h-4 border border-gray-200 flex items-center justify-center text-[9px] ${
                        ["Delivered"].includes(trackResult.status) ? "bg-black text-white" : "bg-white"
                      }`}>✓</div>
                      <p className="text-black uppercase">Delivered Successful</p>
                      <p className="font-sans font-normal text-gray-500 text-[10px] leading-tight">Package verified and payment collected successfully.</p>
                    </div>
                  </div>
                </div>
              </div>
        ...
        {activePage === "blog" && (
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in text-left select-none">
            <div className="border-b border-gray-100 pb-4 mb-8">
              ...
              <p className="text-xs text-gray-500 font-mono mt-0.5">COMPILATIONS, COMPARISONS, AND TIPS</p>
            </div>
            ...
            <article className="space-y-6">
              <div className="border-b border-gray-100 pb-4">
                ...
                <p className="text-[11px] font-mono text-gray-400 mt-2">
                  PUBLISHED: {new Date(selectedBlog.createdAt).toLocaleDateString()} | BY {selectedBlog.author.toUpperCase()}
                </p>
              </div>
        ...
        {activePage === "about" && (
          <div className="max-w-4xl mx-auto px-4 py-8 animate-fade-in text-left select-none">
            <div className="border-b border-gray-100 pb-4 mb-8 text-center">
              ...
              <p className="text-xs text-gray-500 font-mono mt-0.5">OUR STORY, VISION, AND FAQ</p>
            </div>

            <div className="space-y-8 font-sans text-xs">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center border-b border-gray-100 pb-8">
                <div className="space-y-3">
                  <h3 className="font-display font-extrabold text-sm text-black uppercase">WHO WE ARE</h3>
                  <p className="text-gray-600 leading-relaxed">
                    Founded in 2026, EthioPhone is Ethiopia's dedicated, highly professional smartphone and mobile accessories portal. Operating directly from our showroom in Dembel City Center, Addis Ababa, we supply only genuine products backed by authentic local warranties.
                  </p>
                  <p className="text-gray-600 leading-relaxed">
                    Our clients do not need to deal with insecure imports or fake clones. Every phone we sell passes strict local regulatory compliance tests and is verified in-house.
                  </p>
                </div>
                <div className="shadow-sm p-4 bg-gray-50 font-mono text-[11px] space-y-2 border-none">
                  <p className="font-bold text-blue-600 uppercase">⚡ FAST HUB DELIVERY DECLARED</p>
                  <p>Our dispatch couriers reside directly in key sectors (Bole, Kirkos, Lideta) guaranteeing lightning-fast courier delivery directly to your home or office desk.</p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-4 font-sans text-xs divide-y divide-gray-100">
                  <div className="pt-2">
                    <p className="font-bold text-black uppercase">Q: ARE THE PHONES 100% GENUINE AND NEW?</p>
                    <p className="text-gray-500 mt-1">A: Absolutely. We only sell factory-sealed, genuine devices. Every package comes in original box with serial numbers matchable online.</p>
                  </div>
                  <div className="pt-4">
                    <p className="font-bold text-black uppercase">Q: HOW DO I MAKE PAYMENT?</p>
                    <p className="text-gray-500 mt-1">A: We accept Chapa mobile checkouts (Telebirr, CBE Birr, Awash Birr), pre-delivery Bank Transfers, or Cash on Delivery. For COD, you test the device before pay.</p>
                  </div>
                  <div className="pt-4">
                    <p className="font-bold text-black uppercase">Q: HOW LONG DOES DELIVERY TAKE?</p>
                    <p className="text-gray-500 mt-1">A: Within Addis Ababa, delivery takes between 2 to 4 hours. Out-of-Addis orders are dispatched via post-buses and take 24 to 48 hours.</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        {activePage === "contact" && (
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in text-left select-none">
            <div className="border-b border-gray-100 pb-4 mb-8 text-center">
              ...
              <p className="text-xs text-gray-500 font-mono mt-0.5">SUPPORT LINE, OFFICE MAP, AND ENQUIRIES</p>
            </div>
                ...
                <div className="shadow-sm bg-gray-50 h-64 relative flex flex-col justify-between p-4 overflow-hidden border-none">
                  <div className="absolute inset-0 opacity-10 font-mono text-[9px] flex flex-wrap gap-4 overflow-hidden pointer-events-none p-2 select-none">
                    {Array.from({ length: 40 }).map((_, i) => (
                      <span key={i}>BOLE ROAD LAT_9.00° LNG_38.78° STREET_GRID</span>
                    ))}
                  </div>
                  ...
                  <p className="text-[10px] font-mono text-gray-400 relative z-10 uppercase">
                    📍 BOLE ROAD, DEMBEL CITY CENTER, 3RD FLOOR, ADDIS ABABA, ETHIOPIA
                  </p>
                </div>

                <div className="grid grid-cols-2 gap-4 font-mono">
                  <div className="shadow-sm p-3.5 bg-white border-none">
                    <p className="font-bold text-blue-600 block text-[10px]">📞 TELEPHONE HOTLINE</p>
                    <p className="text-black font-bold mt-1 text-xs">{settings.phone || "+251911223344"}</p>
                    <p className="text-[9px] text-gray-400 mt-1">Available 8:30 AM - 7:30 PM</p>
                  </div>
                  <div className="shadow-sm p-3.5 bg-white border-none">
                    <p className="font-bold text-blue-600 block text-[10px]">✉️ SUPPORT EMAIL</p>
                    <p className="text-black font-bold mt-1 text-xs">{settings.email || "info@ethiophone.com"}</p>
                    <p className="text-[9px] text-gray-400 mt-1">Response within 24 Hours</p>
                  </div>
                </div>
              </div>

              {/* Right Column Enquiry Form */}
              <form onSubmit={handleContactSubmit} className="shadow-md p-6 bg-white space-y-4 border-none">
                <h3 className="font-mono font-bold text-xs text-black border-b border-gray-100 pb-2 uppercase tracking-wider">
                  SEND AN ENQUIRY MESSAGE
                </h3>

                {contactSuccessMsg && (
                  <p className="bg-green-50 border border-green-600 text-green-600 p-2 text-xs font-mono">{contactSuccessMsg}</p>
                )}

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                  <div className="space-y-1">
                    <label className="font-mono font-bold text-gray-500">YOUR FULL NAME *</label>
                    <input
                      type="text"
                      required
                  ...
                  <div className="space-y-1">
                    <label className="font-mono font-bold text-gray-500">YOUR PHONE NUMBER *</label>
                    <input
                      type="tel"
                      required
                ...
                <div className="text-xs space-y-1">
                  <label className="font-mono font-bold text-gray-500">EMAIL ADDRESS (OPTIONAL)</label>
                  <input
                    type="email"
                ...
                <div className="text-xs space-y-1">
                  <label className="font-mono font-bold text-gray-500">YOUR DETAILED MESSAGE *</label>
                  <textarea
                    required
                    rows={4}
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in select-none">
            {adminToken ? (
              ...
              <div className="max-w-md mx-auto py-12 text-left">
                <form onSubmit={handleAdminLogin} className="shadow-md p-6 bg-white space-y-4 border-none">
                  <div className="text-center pb-2 border-b border-gray-100">
                    <ShieldAlert className="w-8 h-8 text-blue-600 mx-auto" />
                    <h2 className="font-display font-extrabold text-lg text-black mt-2 uppercase">ADMINISTRATOR SIGN IN</h2>
                    <p className="text-[10px] text-gray-500 font-mono">STAFF CREDENTIALS REQUIRED</p>
                  </div>

                  {adminAuthError && (
                    <p className="bg-red-50 border border-red-600 text-red-600 p-2 text-xs font-mono">{adminAuthError}</p>
                  )}

                  <div className="space-y-1 text-xs">
                    <label className="font-mono font-bold text-gray-500">ADMINISTRATIVE USERNAME</label>
                    <input
                      type="text"
                      required
                  ...
                  <div className="space-y-1 text-xs">
                    <label className="font-mono font-bold text-gray-500">PASSWORD</label>
                    <input
                      type="password"
                      required
                  ...
                  <div className="text-[10px] font-mono text-gray-400 border-t border-dashed border-gray-200 pt-3 text-center">
                    🔒 Seeded Default Credentials:<br />
                    Username: <span className="text-black font-bold">admin</span> | Password: <span className="text-black font-bold">adminpassword</span>
                  </div>
                </form>
              </div>
            )}
      ...
      <Footer
        ...
        businessHours={settings.businessHours || "Monday - Saturday: 8:30 AM - 7:30 PM"}
      />
    </div>
  );
}
